...
public class InvalidQuestionStateException extends BusinessException {
    public InvalidQuestionStateException(Long questionId, QuestionStatus currentStatus, String operation) {
        super(
            "Question %s cannot perform operation '%s' while its status is %s"
                .formatted(questionId, operation, currentStatus),
            ErrorCode.QUESTION_INVALID_STATE);
    }

    public InvalidQuestionStateException(Long questionId, QuestionState currentState, String operation) {
        super(
            "Question %s cannot perform operation '%s' while its state is %s"
                .formatted(questionId, operation, currentState),
            ErrorCode.QUESTION_INVALID_STATE);
    }
}