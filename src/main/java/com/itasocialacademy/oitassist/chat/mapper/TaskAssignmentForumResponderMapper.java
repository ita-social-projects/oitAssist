...
public interface TaskAssignmentForumResponderMapper {
    ...
    @Mapping(
        target = "taskAssignmentId",
        source = "assignment.taskAssignmentId")
    @Mapping(
        target = "responderUserId",
        source = "assignment.responderUserId")
    @Mapping(
        target = "responderEmail",
        source = "candidate.email")
    @Mapping(
        target = "responderFirstName",
        source = "candidate.firstName")
    @Mapping(
        target = "responderLastName",
        source = "candidate.lastName")
    @Mapping(
        target = "assignedByUserId",
        source = "assignment.assignedByUserId")
    @Mapping(
        target = "assignedAt",
        source = "assignment.assignedAt")
